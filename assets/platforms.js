const GULF_PLATFORM_CONFIG = {
  ...
  groups: [
    { id: 'general', title: 'General & Global Boards', platforms: [
      { id: 'bayt', name: 'Bayt.com', type: 'direct', coverage: 'Gulf jobs', buildUrl: ({ role, city, location, country }) => { const config = GULF_PLATFORM_CONFIG.countries[country] || GULF_PLATFORM_CONFIG.countries.uae; const scope = city ? config.baytPath : 'international'; return `https://www.bayt.com/en/${scope}/jobs/${slug(role)}-jobs/${city ? `?location=${encodeURIComponent(location)}` : ''}`; } },
